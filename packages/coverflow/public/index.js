(function(){
  ...
  function buildSocialLinks(cover){
    ...
    if (ig) links.push({ href: ig, aria: 'Instagram', svg: ICON_IG });
    if (tiktok) links.push({ href: tiktok, aria: 'TikTok', svg: ICON_TT });
    if (spotify) links.push({ href: spotify, aria: 'Spotify', svg: ICON_SP });
    if (!links.length) return '';
    return `<div class="social-links">${links.map(l=>`<a class="social-link" target="_blank" rel="noopener" aria-label="${l.aria}" href="${l.href}">${l.svg}</a>`).join('')}</div>`;
  }
...
/* orphaned duplicate from earlier merges removed below (kept for reference)
  ...
  function buildSocialLinks(cover){
    ...
    const spotify = cover.artistDetails?.spotifyLink || cover.music?.url || null;
    const ICON_IG = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64"><path fill="#ffffff" d="M 21.580078 7 C 13.541078 7 7 13.544938 7 21.585938 L 7 42.417969 C 7 50.457969 13.544938 57 21.585938 57 L 42.417969 57 C 50.457969 57 57 50.455062 57 42.414062 L 57 21.580078 C 57 13.541078 50.455062 7 42.414062 7 L 21.580078 7 z M 47 15 C 48.104 15 49 15.896 49 17 C 49 18.104 48.104 19 47 19 C 45.896 19 45 18.104 45 17 C 45 15.896 45.896 15 47 15 z M 32 19 C 39.17 19 45 24.83 45 32 C 45 39.17 39.169 45 32 45 C 24.83 45 19 39.169 19 32 C 19 24.831 24.83 19 32 19 z M 32 23 C 27.029 23 23 27.029 23 32 C 23 36.971 27.029 41 32 41 C 36.971 41 41 36.971 41 32 C 41 27.029 36.971 23 32 23 z"/></svg>';
    const ICON_SP = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0,0,256,256"><g fill="#ffffff" fill-rule="nonzero" stroke="none" stroke-width="1" stroke-linecap="butt" stroke-linejoin="miter" stroke-miterlimit="10" stroke-dasharray="" stroke-dashoffset="0" font-family="none" font-weight="none" font-size="none" text-anchor="none" style="mix-blend-mode: normal"><g transform="scale(3.55556,3.55556)"><path d="M36,10c14.34,0 26,11.66 26,26c0,14.34 -11.66,26 -26,26c-14.34,0 -26,-11.66 -26,-26c0,-14.34 11.66,-26 26,-26zM46.43,45.751c0.21,-0.852 0.027,-1.522 -0.877,-2.121c-0.904,-0.599 -6.631,-3.326 -13.644,-3.326c-3.83,0 -7.216,0.704 -8.331,1.147c-1.22,0.485 -0.978,1.895 -0.578,2.328c0.399,0.434 0.914,0.646 1.674,0.496c0.76,-0.15 3.894,-0.915 7.235,-0.915c3.341,0 7.647,0.621 12.086,3.049c1.327,0.725 2.226,0.195 2.435,-0.658zM49.018,39.319c0.18,-0.745 -0.028,-1.445 -1.127,-2.173c-1.099,-0.728 -7.443,-4.21 -15.973,-4.21c-4.658,0 -7.845,0.886 -9.201,1.424c-1.484,0.589 -1.189,2.331 -0.703,2.859c0.486,0.527 1.111,0.572 2.036,0.389c0.925,-0.183 3.804,-0.927 7.868,-0.927c4.064,0 8.957,0.876 14.491,3.894c1.35,0.736 2.359,-0.218 2.609,-1.256zM52.116,32.258c0.199,-0.825 -0.031,-1.794 -1.248,-2.6c-1.217,-0.806 -9.334,-4.226 -18.78,-4.226c-5.158,0 -9.549,0.76 -11.051,1.356c-1.644,0.653 -1.316,2.892 -0.779,3.476c0.538,0.584 1.23,0.871 2.255,0.669c1.025,-0.202 5.075,-1.15 9.575,-1.15c4.5,0 11.01,0.698 17.139,4.04c1.495,0.816 2.612,-0.416 2.889,-1.565z"></path></g></g></svg>';
    const ICON_TT = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0,0,256,256"><g fill="#ffffff" fill-rule="nonzero" stroke="none" stroke-width="1" stroke-linecap="butt" stroke-linejoin="miter" stroke-miterlimit="10" stroke-dasharray="" stroke-dashoffset="0" font-family="none" font-weight="none" font-size="none" text-anchor="none" style="mix-blend-mode: normal"><g transform="scale(3.55556,3.55556)"><path d="M42.5,14c14.375,0 15.5,1.125 15.5,15.5v13c0,14.375 -1.125,15.5 -15.5,15.5h-13c-14.375,0 -15.5,-1.125 -15.5,-15.5v-13c0,-14.375 1.125,-15.5 15.5,-15.5zM45.616,34.045v-3.79c0,0 -2.003,0.01 -3.695,-1.54c-1.422,-1.302 -1.54,-3.743 -1.54,-3.743h-3.79c0,0 0,12.934 0,14.994c0,2.06 -1.793,3.127 -3.079,3.127c-0.969,0 -3.174,-0.76 -3.174,-3.151c0,-2.51 2.535,-3.15 3.198,-3.15c0.663,0 0.924,0.142 0.924,0.142v-4c0,0 -0.616,-0.059 -1.066,-0.074c-3.825,-0.13 -6.988,3.303 -6.988,7.083c0,3.193 2.497,7.059 7.059,7.059c4.859,0 7.083,-4.052 7.083,-7.035c0,-2.179 0,-7.438 0,-7.438c0,0 1.398,0.782 2.535,1.137c1.136,0.355 2.533,0.379 2.533,0.379z"></path></g></g></svg>';
    if (ig) links.push({ href: ig, aria: 'Instagram', svg: ICON_IG });
    if (tiktok) links.push({ href: tiktok, aria: 'TikTok', svg: ICON_TT });
    if (spotify) links.push({ href: spotify, aria: 'Spotify', svg: ICON_SP });
    if (!links.length) return '';
