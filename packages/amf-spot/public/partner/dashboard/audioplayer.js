...
// Enhanced render comments function with collapsible functionality
...
function addComment(text, timestamp = null) {
  ...
  // Update UI
  renderCommentsEnhanced(trackId);
  
  // Clear input
  document.querySelector('.comment-input').value = '';
  delete document.querySelector('.comment-input').dataset.timestamp;
  document.querySelector('.comment-input').placeholder = 'Add a comment... (Press \'C\' while playing to timestamp)';
...
function playTrack(track) {
  ...
  state.audioElement.play()
    .then(() => {
      ...
      renderCommentsEnhanced(track.id);
    })
    .catch(error => {
      console.error('[AudioPlayer] Error playing track:', error);
...
// Setup keyboard shortcuts for comments and playback
...
function attachEventListeners() {
  ...
  // Comment form submission and timestamp handling
  const commentInput = document.querySelector('.comment-input');
